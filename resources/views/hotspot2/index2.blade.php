				    ...
				    <!--  -----------------------FIN LOGIN------------------------------  -->
				        ...
				        @foreach($parametros as $val)
				         @if($val->parametro == 'REGISTRO_MANUAL' AND $val->valor == 'SI')
				          <div class="input-field col s12">
				            <a href="{{ url('#') }}" type="submit" class="btn waves-effect waves-light gradient-45deg-indigo-blue  col s12" style="height: 44px; padding-top: 0.3rem;letter-spacing: .5px;">Registro Manual
				              <i class="mdi-content-send right"></i>
				            </a>
				          </div>
				          @endif
