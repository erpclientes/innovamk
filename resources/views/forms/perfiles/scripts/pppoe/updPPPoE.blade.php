<script type="text/javascript">
    ...
    @foreach ($pppoe as $val)
      $(document).on('click','#updPPPoE{{$val->idperfil}}', function(){
        ...
        $("#pu_local_address").val($(this).data('local_address'));

        var idrouter = $(this).data('idrouter');
        var perfil = $(this).data('perfil');
        var dsc = $(this).data('dsc_perfil');
        var remote_address = $(this).data('remote_address');
        ...
        $.ajax({
           ...
           success:function(data){
              ...
              $.each(data, function(i, item) {
                  ...
                  if (perfil == item.name || dsc == item.name) {
                    $('#pu_remote_address').append("<option selected value='"+item.name+"'>"+item.name+"</option>");
                  }else{
                    $('#pu_remote_address').append("<option value='"+item.name+"'>"+item.name+"</option>");
                  }
              });

              
              $("#pu_remote_address").val(remote_address);
