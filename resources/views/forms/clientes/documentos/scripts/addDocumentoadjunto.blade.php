				...
				contentType: false,
				 ...
				 success: function(data){
					...
					if ( data[0] == "error") {
						...
						$('#h_error2').text('');
						$('#h_error3').text('');						 
						( typeof data.archivo != "undefined" )? $('#h_error1').text('cargar archivo') && $('#archivo').focus() : null;
					 ( typeof data.iddocumento != "undefined" )? $('#h_error2').text(data.iddocumento) : null;
					 ( typeof data.descripcionAdj != "undefined" )? $('#h_error3').text(data.descripcionAdj) : null;  
					 }	 
					if(data=="TRUE"){
						console.log(data); 
